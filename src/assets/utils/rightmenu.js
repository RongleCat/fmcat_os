      ...
      bind(el, binding, vnode) {
        ...
        el.addEventListener("contextmenu", (e) => {
          ...
          if (!document.getElementById("tt_right_menu" + currentSize)) {
            ...
            if (binding.value['boxClass']) {
              let classNames = binding.value['boxClass']
              let classType = typeof (classNames)
              switch (classType) {
                case 'string':
                  classNames.split(',').forEach(item => {
                    boxDiv.classList.add(item)
                  })
                  break;
                case 'object':
                  if (Array.isArray(classNames)) {
